/**
 * Keyframe / easing math — lifted from the Clypra editor's evaluation core.
 *
 * Source: github.com/AIEraDev/Clypra @ 2e85676f0c56d1e5f28fabcd9a3ab9952442a35b
 *   path: src/core/evaluation/animation.ts
 * License: MIT (see THIRD-PARTY-NOTICES.md at repo root for full text).
 *
 * Changes made in this port:
 * - Dropped `evaluateVisualPropertyKeyframes` and the `VisualPropertyKeyframe`
 *   import: that helper duplicated `evaluateProperty` against a
 *   Clypra-specific type (`@/types`) that doesn't exist in SundayEdit. The
 *   generic `evaluateProperty<T>` below covers the same ground.
 * - No other logic changes — this is pure math with no clip/store coupling,
 *   so it ports verbatim.
 *
 * OSS-INTEGRATION-PROGRAM E1: foundation lift only. Nothing in SundayEdit
 * calls this module yet — it's staged for the upcoming keyframed-transform
 * work (overlay/text animation on the timeline).
 */
 ...
 */
export function isKeyframed<T>(prop: unknown): prop is KeyframedProperty<T> {
  return (
    prop !== null &&
    typeof prop === "object" &&
    "keyframes" in prop &&
    Array.isArray((prop as { keyframes: unknown }).keyframes)
  );
}
...
export function getEasingProgress(
  easing: Keyframe<unknown>["easing"],
  t: number,
...
/**
 * Parses any color format (HEX, RGB, RGBA) into an [r, g, b, a] tuple.
 */
...
/**
 * Interpolates two color strings, returning an rgba(...) string.
 */
...
export function evaluateProperty<T>(
  ...
  if (timeOffset >= sorted[sorted.length - 1].time)
  ...
  const easedProgress = getEasingProgress(
    left.easing,
    progress,
    left.controlPoints,
  );
